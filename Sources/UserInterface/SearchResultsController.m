...
didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([appDelegate canConnectToInternet])
    {
        ...
        if (indexPath.section == 1)
        {
            ...
            {
                destination = [NSString stringWithFormat:[Stockboy getURIForKey:@"Google URL"],
                                                         gLastSearchString];
            }
        }
        else
...
- (void)startSearchThreadWithQuery:(NSString *)query
{
    ...
    gRefreshThread = [[[NSThread alloc] initWithTarget:self
                                              selector:@selector(threadRefreshHits:)
                                                object:query] autorelease];
...
shouldReloadTableForSearchString:(NSString *)searchString
{
    ...
    gRefreshTimer = [NSTimer scheduledTimerWithTimeInterval:0.02
                                                     target:self
           ...
           returningAtMost:(NSInteger)maxHits
{
    ...
    for (NSDictionary *item in items)
    {
        ...
        {
            titleRange = [[item objectForKey:@"title"] rangeOfString:term
                                                             options:NSCaseInsensitiveSearch];
            urlRange = [[item objectForKey:@"url"] rangeOfString:term
                                                         options:NSCaseInsensitiveSearch];
