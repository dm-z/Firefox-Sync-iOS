...
static BOOL IsNativeAppURLWithoutChoice(NSString *link) {
    if (link != nil)
    {
        ...
        if (url != nil)
        {
            ...
            {
                nativeSchemes = [[NSSet setWithObjects:@"mailto",
                                                       @"tel",
                                                       @"sms",
                                                       @"itms",
                                                       nil] retain];
           ...
           withTitle:(NSString *)title
{
    ...
    if (_locationURL == nil)
    {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Error Loading Page", @"error loading page")
                                                        message:NSLocalizedString(@"Invalid URL", @"invalid url")
                                                       delegate:nil
...
- (IBAction)exportURL:(id)sender
{
    ...
    NSMutableString *safeURLString = [NSMutableString stringWithFormat:@"%@://%@",
                                                                       [rawURL scheme],
                                                                       [rawURL host]];
...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation
{
    ...
    [appDelegate rotateFullscreenView:appDelegate.tabBarController.view
      ...
      withTitle:(NSString *)title
{
    if (IsBlockedURL(destination))
    {
        ...
        NSDictionary *errInfo = [NSDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"Cannot Load Page", @"unable to load page"),
                                                                           @"title",
                                                                           NSLocalizedString(@"SyncClient cannot load the page because it is of an unsupported type.", "SyncClient cannot load the page because it is of an unsupported type."), @"message",
                                                                           nil];
